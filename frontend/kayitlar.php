...
// Kategorileri getir
if ($ana_kategori_kolon_var) {
    $where_clause = $where_sql ? $where_sql . " AND COALESCE(ana_kategori, kategori) IS NOT NULL" : "WHERE COALESCE(ana_kategori, kategori) IS NOT NULL";
    $kategori_sql = "SELECT DISTINCT COALESCE(ana_kategori, kategori) as ana_kategori FROM cti_kayitlari $where_clause ORDER BY ana_kategori";
    ...
    if ($ana_kategori_filtre) {
        $alt_kategori_sql = "SELECT DISTINCT alt_kategori FROM cti_kayitlari WHERE ana_kategori = ? AND alt_kategori IS NOT NULL ORDER BY alt_kategori";
    ...
    $kategori_sql = "SELECT DISTINCT kategori FROM cti_kayitlari WHERE kategori IS NOT NULL ORDER BY kategori";
    $stmt = $db->prepare($kategori_sql);
    $stmt->execute();
    $ana_kategoriler = $stmt->fetchAll(PDO::FETCH_COLUMN);
    $alt_kategoriler = [];
...
<head>
    ...
    <title>Dvice CTI - Kayıtlar</title>
...
<body>
    ...
    <div class="container">
        <div style="margin-bottom: 24px; text-align: center;">
            <h2 style="color: var(--color-primary); font-size: 24px; margin-bottom: 8px; text-shadow: 0 0 8px rgba(57, 255, 20, 0.4);">Tüm Kayıtlar</h2>
            <p style="color: var(--color-text-muted); font-size: 14px; margin-bottom: 16px;">Filtreleme ve arama ile kayıtları görüntüleyin</p>
            ...
            <form method="GET" action="" id="filtre-form">
                ...
                <div class="filtre-grup">
                    ...
                    <select name="kritiklik">
                        <option value="">Tümü</option>
                        <option value="dusuk" <?= $kritiklik_filtre === 'dusuk' ? 'selected' : '' ?>>Düşük</option>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="width: 38%">Başlık / İçerik Özeti</th>
                        ...
                        <?php foreach ($kayitlar as $kayit): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <br>
                                    <small style="color: var(--color-text-muted);"><?= guvenliCikti(substr($kayit['ozet'], 0, 120)) . '...' ?></small>
                                </td>
                                <td>
                                    <?php if (isset($kayit['ana_kategori']) && $kayit['ana_kategori']): ?>
                                        ...
                                        <?php if (isset($kayit['alt_kategori']) && $kayit['alt_kategori']): ?>
                                            <br><small style="color: var(--color-text-muted);"> - <?= guvenliCikti($kayit['alt_kategori']) ?></small>
                                        <?php endif; ?>
                                    <?php else: ?>
                                ...
                                <td style="text-align: center;">
                                    <a href="detay.php?id=<?= $kayit['id'] ?>" class="detay-link">İNCELE</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
    ...
    <script>
        ...
        document.getElementById('temizle-btn').addEventListener('click', function(e) {
            window.location.href = 'kayitlar.php?temizle=1';
        });
